...
+ (instancetype)sharedBrightnessView {
    static ZXBrightnessView *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[ZXBrightnessView alloc] init];
...
- (instancetype)init {
    if (self = [super init]) {
        self.frame = CGRectMake(kMediaPlayerScreenWidth * 0.5, kMediaPlayerScreenHeight * 0.5, 155, 155);
        ...
        self.backImage = ({
            ...
            imgView.image        = MediaPlayerImage(@"MediaPlayer_brightness");
...
- (void)layoutSubviews {
    ...
    self.center = CGPointMake(kMediaPlayerScreenWidth * 0.5, kMediaPlayerScreenHeight * 0.5);
}
...
//    [[window getCurrentViewController] setNeedsStatusBarAppearanceUpdate];
}
